...
const UpdateProduct = async (req, res) => {
    const { name,images,description,category,price,rating,thumbnail,_id } = req.body

    const filter = {_id}
    const target = {name,images,description,category,price,rating,thumbnail}
...
const DeleteProduct = async (req, res) => {
    ...
    try {
        await connect(process.env.MONGO_URL)
        const products = await Product.deleteOne({ title })
        res.json({
